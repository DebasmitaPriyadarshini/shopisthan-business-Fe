...
import { StoreProfileCardSkeleton } from "../SkeletonComponents/StoreComponents";

export const StoreViewProfileCard = (props) => {
  const { store, checkedAlreadyStoreDetails, storeLoading } = props;
  const history = useHistory();
  const [shareModal, setShareModal] = useState(false);

  const storeMemberDate = (date) => {
    const mlist = [
      "Jan",
      "Feb",
      "Mar",
    ...
    if (mon.charAt(0) === "0") {
      const mou = Number(mon.charAt(1)) - 1;
      return mlist[mou] + " " + day + "," + y;
    } else {
      return mlist[Number(mon) - 1] + " " + +day + "," + y;
    }
  };
  ...
  return (
    <>
            ...
            <div className="respo-flex-div-with-tore-profile-and-othjer">
              <div className="this-div-for-profile">
                ...
                store.storeProfilePicture.img ? (
                  ...
                  <h2 className="shopfisrtcharacterinshopprofile">
              ...
              <div className="new-store-pro-fil-e-div-for-flec-inrespo">
                <div className="user-shop-name-in-store-card">
                  <h2
                    onClick={() => {
                      const alreadyStoreDetails =
                        checkedAlreadyStoreDetails &&
                        checkedAlreadyStoreDetails.location &&
                        checkedAlreadyStoreDetails.location.state &&
                        checkedAlreadyStoreDetails.location.state.ProductDetails
                          ? true
                          : false;
                      history.push({
                        pathname: `/${
                  ...
                  <div className="share-and-bell-icon-of-store-cnis">
                    <img
                      style={{ height: "18px" }}
                      src={ImportImage.BelliconNotify}
                      alt="Bell-Icon"
                    />
                    <img
                      style={{ height: "20px" }}
                ...
                <div className="user-shop-name-in-store-card-dedfefa">
                  <div className="floowers-and-all-otjher-infoe-background-gey">
                    <p style={{ fontSize: "12px" }}>
                      {store && store.storeCity ? store.storeCity : "---"}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="member-since">
              <p>
                ...
                {store && store.createdAt
                  ? storeMemberDate(store.createdAt)
                  : null}
              </p>
            </div>
          </div>
      ...
      <ShareModal
        ...
        shareLink={`${
          store && store.storeUrl ? store.storeUrl.split(" ").join("-") : "Shop"
        }`}
        shareTitle={`Have a look at  ${
          store && store.storeName ? store.storeName : "Shop"
        } Store at Shopisthan`}
      />
    </>
  );
